...
module.exports = function(botkit, config) {
    ...
    bot.startRTM = function(cb) {
        bot.api.rtm.start({
            ...
            for (var user in res.users) {
                var tmpUserId = res.users[user].id;
                var tmpUser = res.users[user]
                
                bot.users_info[tmpUserId] = tmpUser;
            }
    ...
    bot.getUserById = function (userId, cb) {
        if (bot.users_info && bot.users_info[userId]) {
            return cb(null, bot.users_info[userId]);    
        } else {
            cb("Unknown User!");
        }
